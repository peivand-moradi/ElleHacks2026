...
export default function App() {
  ...
  const [aiMessage, setAiMessage] = useState(
    "Let's get cooking! We need coins and a good credit score for this pizza."
  );
  const [isTyping, setIsTyping] = useState(false);

  // store only primitive snapshots for reliable comparisons
  ...
  // --- GEMINI TRIGGER LOGIC (minimal change) ---
  useEffect(() => {
    const triggerAi = async (action) => {
      setIsTyping(true);
      const msg = await getInnerVoiceAdvice(action, state);
    ...
    const invLen = state.inventory.length;
    const walletCoins = state.walletCoins;
    const creditDebt = state.creditDebt;
    const creditLimit = state.creditLimit;

    if (invLen > prev.invLen) {
      triggerAi("Bought a new pizza ingredient using credit!");
    } else if (creditDebt < prev.creditDebt) {
      triggerAi("Paid off some credit card debt!");
    } else if (walletCoins > prev.walletCoins) {
      const diff = walletCoins - prev.walletCoins;
      triggerAi(`Earned ${diff} coins! Let's use that dough to pay down debt.`);
    } else {
      // only trigger when crossing into "maxed out"
      const wasMaxed = prev.creditDebt >= prev.creditLimit && prev.creditDebt > 0;
      const isMaxed = creditDebt >= creditLimit && creditDebt > 0;
      if (!wasMaxed && isMaxed) {
        triggerAi("Our credit card is maxed out! Time to pay before more toppings.");
      }
    }

    prevSnap.current = { invLen, walletCoins, creditDebt, creditLimit };
  }, [state.inventory.length, state.walletCoins, state.creditDebt, state.creditLimit]);
      ...
      <div className="page-wrapper">
        ...
        {playing && gameId === "whack" && game && (
          <WhackACoin
            ...
            onFinish={async (earned) => {
              setState((s) => earnCoins(s, earned));
              setPage("games");
              // Gemini message now updates via useEffect (removed duplicate trigger)
            }}
          />
        )}
        ...
        {playing && gameId === "tictactoe" && game && (
          <TicTacToe
            ...
            onFinish={async (moves, winner) => {
              const earned =
                winner === "X" ? game.reward : winner === "Draw" ? Math.round(game.reward / 2) : 0;
              setState((s) => earnCoins(s, earned));
              setPage("games");
              // Gemini message now updates via useEffect (removed duplicate trigger)
            }}
        ...
        {/* --- GEMINI UI BUBBLE --- */}
        <div className="inner-voice-container">
          <div className="bubble">{isTyping ? "Thinking..." : aiMessage}</div>
          <div className="avatar-circle">🧠</div>
        </div>
      </div>

      {payOpen && (
        <PayCreditModal state={state} setState={setState} onClose={() => setPayOpen(false)} />
      )}
    </div>
  );
}
